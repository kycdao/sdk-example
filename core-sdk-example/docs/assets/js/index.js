...
const mintingOptionsSetup = () => {
  ...
  mintButton.addEventListener("click", async () => {
    ...
    if (imageId) {
      ...
      try {
        const mintingResult = await kycDao.startMinting(mintingData);
        status.innerHTML = `Minting successful - <a href="${mintingResult.transactionUrl}">Transaction details</a><br \><img class="nft-image" src="${mintingResult.imageUrl}">`;
      } catch (e) {
        status.innerHTML = e;
      }
    } else {
...
const initKycdao = async (chain) => {
  ...
  const sdkMintedNftImage = document.getElementById("minted-nft-image");
  ...
  try {
    ...
    window.kycDaoTransactionUrl =
      kycDaoInitResult.mintingResult?.transactionUrl;
    window.kycDaoNftImageUrl = kycDaoInitResult.mintingResult?.imageUrl;
    ...
    sdktransactionUrl.innerHTML = kycDaoTransactionUrl
      ...
      : "None";
    sdkMintedNftImage.innerHTML = kycDaoNftImageUrl
      ? `<img class="nft-image" src="${kycDaoNftImageUrl}" />`
      : "None";
  } catch (e) {
